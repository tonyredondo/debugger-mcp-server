...
internal static class LlmSystemPrompts
{
        ...
        bool agentConfirmationEnabled)
    {
        ...
        sb.AppendLine("- Before requesting exec tool calls, determine the active debugger type (LLDB vs WinDbg) from the CLI context and/or the crash report JSON metadata (metadata.debuggerType), and only issue commands that exist in that debugger.");
        sb.AppendLine("- Never run WinDbg-only native commands in an LLDB session (or vice versa). Exception: WinDbg-style SOS commands prefixed with '!' (e.g., !pe, !clrstack, !dumpheap) are acceptable in LLDB sessions because the server strips the leading '!'.");
        ...
        sb.AppendLine("- If metadata.sosLoaded=true, NEVER attempt to load SOS and NEVER claim SOS is not loaded. Do not run any \"plugin load libsosplugin.so\", \".load sos\", or similar commands.");
        sb.AppendLine("  - Prefer SOS commands via exec \"!<command> ...\" for portability (e.g., exec \"!clrthreads\", exec \"!pe\", exec \"!clrstack -a\", exec \"!dumpheap -stat\"). On LLDB the server strips the leading '!'.");
        sb.AppendLine("  - If the environment supports it, exec \"sos <command> <args>\" is also acceptable (e.g., exec \"sos help\", exec \"sos dumpil ...\").");
